...
function extract(markdownStr, markdownFileName, skeletonFilename, srcLang, trgLang, extraHtmlTags) {
    ...
    function onKeyValueObject(kv) {
        Object.keys(kv).forEach(key => {
            if (typeof kv[key] === 'object') {
                ...
                kv[key] && addUnit(kv[key], xmlEscape(kv[key]));
            }
        });
    }
    ...
    function onCode(code, lang) {
        ...
        if (lang === 'lua') {
            const luaSingleLineCommentRegexp = /--\s([\s\S].*)|(---+)/g;
            while ((comments = luaSingleLineCommentRegexp.exec(code)) !== null) {
                if (comments[1]) {
                    onText(comments[1]);
                }
            }
        ...
        // imho, it is easier to translate `mermaid` directly instead of extracting texts :(
        // So let `mermaid` pass to the `xliff line by line
        if (lang === 'mermaid') {
            let line;
            const mermaidLineRegexp = /([^\s].*)/g;
            while ((line = mermaidLineRegexp.exec(code)) !== null) {
                if (line[1]) {
                    onText(line[1]);
                }
            }
    ...
    function getXliffAndSkeletonForInlineNode(node, markdownString, inline = { xliff: [], skeleton: [] }) {
        ...
        switch (node.type) {
            ...
            case 'inlineCode':
                ...
                inline.xliff.push(getTag('bpt', ++i, mark), xmlEscape(node.value), getTag('ept', i, mark));
                break;
        }
